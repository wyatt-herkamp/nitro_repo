...
impl StorageManager {
    pub async fn init() -> Result<StorageManager, StorageHandlerError> {
        #[cfg(feature = "multi_storage")]
        {
            let storage_controller = MultiStorageController::init()
                .await
                .map_err(StorageHandlerError::MultiStorageError)?;
...
impl Serialize for StorageManager {
    ...
    {
        match self {
            MultiStorageHandler(handler) => serializer.serialize_some(handler),
        }
    }
}
...
impl StorageHandlerType for StorageManager {
    ...
    async fn get_storage_by_name(&self, name: &str) -> Result<Option<Storage>, Self::Error> {
        match self {
            #[cfg(feature = "multi_storage")]
            MultiStorageHandler(handler) => handler
