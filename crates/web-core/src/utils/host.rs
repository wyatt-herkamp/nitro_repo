//! Reading the host, scheme and origin a request was addressed to.
//!
//! Pure header parsing: every function here takes exactly the values it reads, so none of it
//! depends on the application state. That is what lets a repository type ask "what host was this
//! addressed to?" without reaching into the site — see the accessors on `SiteContext`, which are
...
use crate::utils::header::HeaderMapExt;
...
pub fn request_origin(
    ...
    match request_authority(headers, uri, trust_forwarded) {
        Some(authority) => {
            let scheme = request_scheme(headers, uri, trust_forwarded, app_url);
            Some(format!("{scheme}://{authority}"))
        }
        None => {
            let app_url = app_url.trim_end_matches('/');
            (!app_url.is_empty()).then(|| app_url.to_owned())
        }
    }
}
