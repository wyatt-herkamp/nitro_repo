...
impl AuthenticationRaw {
    #[instrument(skip(header, sessions), fields(project_module = "Authentication"))]
    pub fn new_from_header(header: AuthorizationHeader, sessions: &SessionManager) -> Self {
        match header {
            ...
            AuthorizationHeader::Session { session } => match sessions.get_session(&session) {
    ...
    pub fn new_from_cookie(cookie: &Cookie<'static>, sessions: &SessionManager) -> Self {
        match sessions.get_session(cookie.value()) {
            Ok(Some(ok)) => AuthenticationRaw::Session(ok),
            Err(err) => {
                error!("Failed to get session: {}", err);
                AuthenticationRaw::NoIdentification
